...
2. MRT 仅作为配置占位量保留，当前执行模型未实现 maximum residence time 约束或相应标准丢弃行为；
...
2. 规则库参数尚未系统标定；
3. MRT 仅作为配置占位量保留，未在当前执行模型中约束 residence time 或触发 MRT drop；
...
MRT 涉及 ATS 标准中的 residence time 和丢弃行为，当前单跳 Python PoC 不应把配置占位值误写成已执行的标准机制。因此中期阶段只聚焦 CIR/CBS，MRT 作为理论变量保留，并计划在 OMNeT++/INET 阶段实现和验证。
...
MRT 仅保留为理论/配置占位量；当前 Python 执行模型不施加 residence-time/MRT drop 约束。

这样做的原因是：

1. CIR/CBS 更容易映射到当前 PoC 的令牌桶整形逻辑；
2. MRT 涉及最大驻留时间、丢弃策略和标准 ATS 行为，当前 PoC 未执行该语义；
