...
> 更新说明：根据文献阅读结果，将早期 `r/b` 表述统一修正为 `CIR/CBS/MRT`。当前阶段规则库只调整 CIR/CBS，MRT 固定。
...
| 令牌桶水位 | $\tau/CBS$ | 判断资源是否过剩 | $\theta_\tau^{full}, \theta_\tau^{empty}$ |
...
| R6 | 稳态回归 | 连续 $W$ 个窗口无 R1–R5 触发 | $(CIR,CBS)$ 向默认/离线最优配置缓慢回归 | 防止动态事件后长期过度配置 |
...
2. **为什么当前只调整 CIR/CBS？**  
   MRT 涉及最大驻留时间与丢弃策略，短期先固定，避免引入标准 ATS 的复杂实现；中期后扩展。
