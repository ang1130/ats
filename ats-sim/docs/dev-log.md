...
**不是严谨的最终实验结果**，暂不可直接用作中期答辩最终数据。原因：
...
4. **静态默认参数 (r=8Mbps, b=20Kbit) 是为制造违约而调的**，非离线标定结果（M8 标定尚未做）
5. **尚未接入规则库** (M7)，"规则法 vs 静态"对比还没做
...
**正式实验数据需待：** M7 规则库 + M8 离线标定完成后，且参数经标定而非人为试凑，方可用作答辩数据。
中期答辩时应坦诚说明这是"初步验证"，全栈仿真 (OMNeT++) 列为中期后工作。
...
| `docs/ats-rulelib-and-schedule.md` | 由 v1.0 更新为 v1.1，将规则动作改为 `CIR/CBS` 调整，并说明 MRT 暂固定 |
...
- 配置来源：`traffic_literature.yaml` + `scenario_literature.yaml`
- 搜索变量：CIR/CBS
- MRT：固定为 `traffic_literature.yaml` 中的 `ats.mrt_fixed`，当前 Python PoC 不搜索 MRT
- 搜索空间：
